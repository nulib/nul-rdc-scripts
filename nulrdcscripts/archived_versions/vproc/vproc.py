...
def batch_video(input, output):
    ...
    for item in os.listdir(input):
        ...
        # check if it's a folder and hasn't been transcoded yet
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all jobs
        future_to_item = {
            executor.submit(single_video, item, item): item 
        ...
        for future in as_completed(future_to_item):
            ...
            try:
                future.result()
                print(f"[{completed}/{len(items_to_process)}] Completed: {os.path.basename(item)}")
            except Exception as exc:
                print(f"[{completed}/{len(items_to_process)}] Error processing {os.path.basename(item)}: {exc}")
