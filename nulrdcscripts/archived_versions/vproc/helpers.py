...
def generate_coding_history(coding_history, hardware, append_list):
    ...
    """
    # Return early if hardware is None or empty
    if not hardware:
...
def import_csv(csvInventory):
    ...
    try:
        with open(csvInventory, encoding="utf-8") as f:
            ...
            # Define which fields are encoding chain related
            encoding_chain_fields = [
                "VTR used", "VTR output used", "tape brand", 
                "tape record mode", "TBC used", "TBC output used", 
                "ADC", "capture card"
            ]
            ...
            for field in video_fieldnames_list:
                ...
                # uses field[0] so when it tells user which ones are missing
                # they will use our current standard
                if not field[0] in fieldnames:
                    missing_fieldnames.append(field[0])

            # Check if we can proceed with missing fields
            if not missing_fieldnames:
                # All fields found, proceed normally
                pass
            elif args.no_encoding_chain:
                # Only allow missing encoding chain fields when flag is used
                non_encoding_missing = [f for f in missing_fieldnames 
                                       if f not in encoding_chain_fields]
                
                if not non_encoding_missing:
                    print("WARNING: Encoding chain fields missing from CSV.")
                    print("Processing will continue without encoding chain metadata.")
                    # Mark encoding chain fields as intentionally missing
                    for field in missing_fieldnames:
                        fieldnames[field] = None
                    missing_fieldnames = []  # Clear since we're handling it
                else:
                    print(f"ERROR: Required fields missing: {non_encoding_missing}")
                    print("Use --no_encoding_chain / -nec flag only when encoding chain fields are missing")
                    quit()
            elif "filename" not in missing_fieldnames:
                # Without the flag, we require all fields or offer file list mode
                print("WARNING: Unable to find all column names in csv file")
                print(f"Missing fields: {missing_fieldnames}")
                print("If these are encoding chain fields (VTR, TBC, ADC, capture card),")
                print("use --no_encoding_chain or -nec flag to process without them.")
                print("")
                print("File name column found. Interpret csv file as file list instead? (y/n)")
                yes = {"yes", "y", "ye", ""}
                no = {"no", "n"}
                choice = input().lower()
                if choice in yes:
                ...
                print("ERROR: No matching column names found in csv file")
                print(f"Missing fields: {missing_fieldnames}")
                quit()
            ...
            if not missing_fieldnames:
                for row in reader:
                    ...
                    # Handle encoding chain fields - use empty string if field not found
                    vtr = row.get(fieldnames.get("VTR used", ""), "") if fieldnames.get("VTR used") else ""
                    vtrOut = row.get(fieldnames.get("VTR output used", ""), "") if fieldnames.get("VTR output used") else ""
                    tapeBrand = row.get(fieldnames.get("tape brand", ""), "") if fieldnames.get("tape brand") else ""
                    recordMode = row.get(fieldnames.get("tape record mode", ""), "") if fieldnames.get("tape record mode") else ""
                    tbc = row.get(fieldnames.get("TBC used", ""), "") if fieldnames.get("TBC used") else ""
                    tbcOut = row.get(fieldnames.get("TBC output used", ""), "") if fieldnames.get("TBC output used") else ""
                    adc = row.get(fieldnames.get("ADC", ""), "") if fieldnames.get("ADC") else ""
                    dio = row.get(fieldnames.get("capture card", ""), "") if fieldnames.get("capture card") else ""
