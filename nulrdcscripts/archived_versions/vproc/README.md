# vproc
A script for transcoding video files. This is an archived version. It allows for MOV files and also mov files that will not have an encoding chain.
...
```
poetry run arcvproc -i INPUT_PATH
```
...
`--no_encoding_chain`, `-nec`
	for videos that will not have an encoding chain. Only works if this is used. Otherwise will throw error.
