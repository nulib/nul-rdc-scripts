...
import nulrdcscripts.vproc.assists as assists
...
def main():
    ...
    if not args.skip_qcli:
        setup.qcli_check()
    setup.mediaconch_check()
    setup.ffprobe_check()
    global ffvers
    ffvers = setup.get_ffmpeg_version()

    # verify that mediaconch policies are present
    setup.mediaconch_policy_exists(movPolicy)
    setup.mediaconch_policy_exists(mkvPolicy)
...
def single_video(input, output):
    for mkvFile in glob.glob1(input, "*.mkv"):
        ...
        new_path = os.path.join(input,'/p')
        presPath =os.rename(src_path, new_path)
        presPath = os.path.abspath(presPath)
        inputAbsPath = os.path.join(input, mkvFile)
        ...
        pmMD5AbsPath = os.path.join(presPath, mkvBaseFilename + ".md5")
        ...
        inventoryCheck = setup.inventory_check(item_csvDict)
        ...
        if os.path.isfile(inputAbsPath):
            ...
            mkvHash = assists.hashlib_md5(inputAbsPath)
            with open(pmMD5AbsPath, "w", newline="\n") as f:
                print(mkvHash, "*" + mkvFile, file=f)


            # create a dictionary with the mediaconch results from the MKV files
            ...
            mediaconchResults = setup.parse_mediaconchResults(
                mediaconchResults_dict
            )

            # run ffprobe on the output file
            output_metadata = helpers.ffprobe_report(
                mkvFile, outputAbsPath
            )
            # log system info
            ...
            qcResults = helpers.qc_results(
                inventoryCheck, mediaconchResults
            )
            ...
            if not args.skip_ac:
                ...
                acHash = assists.hashlib_md5(acAbsPath)
                with open(
                    os.path.join(
