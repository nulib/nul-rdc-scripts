...
def generatereport(filename, input_path, ffprobe_path):
    ...
    if ext in videoExt:
        ...
        pixel_format = [stream.get("pix_fmt") for stream in (video_output["streams"])][
            0
        ]
        sar = [
            stream.get("sample_aspect_ratio") for stream in (video_output["streams"])
        ][0]
        dar = [
            stream.get("display_aspect_ratio") for stream in (video_output["streams"])
        ][0]
        frame_rate = [
            stream.get("avg_frame_rate") for stream in (video_output["streams"])
        ][0]
        color_space = [
            stream.get("color_space") for stream in (video_output["streams"])
        ][0]
        color_range = [
            stream.get("color_range") for stream in (video_output["streams"])
        ][0]
        color_transfer = [
