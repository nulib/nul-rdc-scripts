...
def videoReport (filename,ffprobe_path,input_file_abspath):
    ...
    generalreport = generalreport(filename, ffprobe_path,input_file_abspath)

def generalreport (filename,ffprobe_path,input_file_abspath):
    audio_output = json.loads(subprocess.check_output([ffprobe_path, "-v","error","-select_streams","a","-show_entries","stream=codec_long_name,bits_per_raw_sample,sample_rate,channels", input_file_abspath,"-of","json",]).decode("ascii").rstrip())
    format_output = json.loads(subprocess.check_output([ffprobe_path,"-v","error","-show_entries","format=duration,size,nb_streams",input_file_abspath,"-of","json",]).decode("ascii").rstrip())
    data_output = json.loads(subprocess.check_output(["ffprobe_path","-v","error","-select_stream","d","-show_entries","stream=codec_tag_string",input_file_abspath, "-of","json"]).decode("ascii").rstrip())
    attachment_output = json.loads(subprocess.check_output([ffprobe_path, "-v","error","-select_streams","t","-show_entries","stream_tags=filename",input_file_abspath,"-of","json"]).decode("ascii").rstrip())
