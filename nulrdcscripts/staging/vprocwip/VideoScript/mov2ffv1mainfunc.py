...
from VideoScript import SetUp as SetUp
...
def aja_mov2ffv1_main():
    ...
    indir = SetUp.InputCheck.input_check()
    outdir = SetUp.OutputCheck.output_check()
    ...
    if not args.skip_qcli:
        SetUp.QcliCheck.qcli_check()
    SetUp.MediaConchCheck.mediaconch_check()
    SetUp.FFProbeCheck.ffprobe_check()
    ffvers = SetUp.FFMPEGCheck.get_ffmpeg_version()

    # verify that mediaconch policies are present
    SetUp.MediaConchPolicyCheck.mediaconch_policy_exists(mkvPolicy)
    ...
    csvDict = SetUp.importcsv.import_csv(csvInventory)
    ...
    for movFilename in glob.glob1(indir, "*.mov"):
        ...
        if os.path.isfile(outputAbsPath):
            ...
            mov_stream_sum = mov2ffv1supportfuncs.checksum_streams(
                inputAbsPath, audioStreamCounter
            )
            mkv_stream_sum = mov2ffv1supportfuncs.checksum_streams(
                outputAbsPath, audioStreamCounter
            )
            # PASS/FAIL - check if input stream md5s match output stream md5s
            streamMD5status = mov2ffv1passfail_checks.stream_md5_status(
            ...
            mediaconchResults_dict = {
                "MOV Mediaconch Policy": mov2ffv1supportfuncs.mediaconch_policy_check(
                    inputAbsPath, movPolicy
                ),
                "MKV Implementation": mov2ffv1supportfuncs.mediaconch_implementation_check(
                    outputAbsPath
                ),
                "MKV Mediaconch Policy": mov2ffv1supportfuncs.mediaconch_policy_check(
                    outputAbsPath, mkvPolicy
                ),
            }
            # PASS/FAIL - check if any mediaconch results failed and append failed policies to results
            mediaconchResults = mov2ffv1passfail_checks.parse_mediaconchResults(
                mediaconchResults_dict
            )

            # run ffprobe on the output file
            output_metadata = mov2ffv1supportfuncs.ffprobe_report(
                mkvFilename, outputAbsPath
            )
            # log system info
            systemInfo = mov2ffv1supportfuncs.generate_system_log(
                ffvers, tstime, tftime
            )

            # PASS/FAIL - are files lossless
            losslessCheck = mov2ffv1passfail_checks.lossless_check(
                input_metadata, output_metadata, streamMD5status
            )

            # create a dictionary containing QC results
            qcResults = mov2ffv1supportfuncs.qc_results(
            ...
            mov2ffv1supportfuncs.create_json(
                jsonAbsPath,
                systemInfo,
                input_metadata,
                mov_stream_sum,
            ...
            if not args.skip_ac:
                ...
                mov2ffv1supportfuncs.two_pass_h264_encoding(
            ...
            csvWriteList = [
                ...
                acFilename,
                mkvFilename,
                mov2ffv1supportfuncs.convert_runtime(
                    output_metadata["file metadata"]["duration"]
                ),
            ]

            # Add QC results to QC log csv file
            mov2ffv1supportfuncs.write_output_csv(
                outdir, csvHeaderList, csvWriteList, output_metadata, qcResults
            )
