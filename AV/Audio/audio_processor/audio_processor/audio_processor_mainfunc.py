...
def audio_processor_main():
    ...
    bwf_policy = os.path.join(
        os.path.dirname(__file__), "data/mediaconch_policies/wav-bwf.xml"
    )
    ...
    reference_inventory_file = os.path.join(
        os.path.dirname(__file__), "data/inventory_reference.csv"
    )
    reference_inventory_list = audio_processor_supportfuncs.load_reference_inventory(
    ...
    if args.source_inventory:
        ...
        source_inventory_dict = audio_processor_supportfuncs.import_inventories(
    ...
    for object in object_list:
        ...
        for file in glob.glob1(object_folder_abspath, "*" + preservation_extension):
            pm_file_abspath = os.path.join(object_folder_abspath, file)
            ...
            ac_file_abspath = os.path.join(
                object_folder_abspath, base_filename + ac_identifier + access_extension
            )
            ...
            loaded_metadata = audio_processor_supportfuncs.load_item_metadata(
            ...
            if args.spectrogram:
                ...
                sox_spectrogram_command = [
                    ...
                    "-o",
                    os.path.join(
                        object_folder_abspath, base_filename + "spectrogram_s.png"
                    ),
                ]
            ...
            mediaconchResults_dict = {
                "Preservation Format Policy": audio_processor_supportfuncs.mediaconch_policy_check(
                    pm_file_abspath, p_wav_policy
                ),
                "Preservation BWF Policy": audio_processor_supportfuncs.mediaconch_policy_check(
                    pm_file_abspath, bwf_policy
                ),
                "Access Format Policy": audio_processor_supportfuncs.mediaconch_policy_check(
                    ac_file_abspath, a_wav_policy
                ),
                "Access BWF Policy": audio_processor_supportfuncs.mediaconch_policy_check(
                    pm_file_abspath, bwf_policy
                ),
            }
            # PASS/FAIL - check if any mediaconch results failed and append failed policies to results
            mediaconchResults = (
                audio_processor_supportfuncs.parse_mediaconchResults(
                    mediaconchResults_dict
                )
            )
            ...
            qcResults = audio_processor_supportfuncs.qc_results(
            ...
            if args.write_json:
                ...
                bwf_meta_dict = audio_processor_supportfuncs.get_bwf_metadata(
                    pm_file_abspath
                )
            ...
            csvWriteList = [
                ...
                None,
                None,
                audio_processor_supportfuncs.convert_runtime(
                    input_metadata["file metadata"]["duration"]
                ),
            ]

            # Add QC results to QC log csv file
            audio_processor_supportfuncs.write_output_csv(
